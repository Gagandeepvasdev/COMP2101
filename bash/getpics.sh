...
# Task 1: Retrieve and install the files kept in the https://zonzorp.net/pics.tgz tarfile
...
#   - Delete the local copy of the tarfile if the extraction was successful
...
# Task 2: Retrieve and install the files kept in the https://zonzorp.net/pics.tgz tarfile
#   - Test to make sure the download and extraction commands work
#   - Delete the local copy of the tarfile if the extraction was successful

# Check if the tarfile exists before downloading it
if ! [ -e ~/public_html/pics/pics.tgz ]; then
    wget -q -O ~/public_html/pics/pics.tgz http://zonzorp.net/pics.tgz

    # Test if the download and extraction commands work
    if tar -xzf ~/public_html/pics/pics.tgz -C ~/public_html/pics; then
        rm ~/public_html/pics/pics.tgz
    else
        # Remove the tarfile if the extraction failed
        rm ~/public_html/pics/pics.tgz
